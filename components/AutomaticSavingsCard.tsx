/**
 * AutomaticSavingsCard Component
 * 
 * Displays automatic savings made today with amount and percentage
 */
...
interface AutomaticSavingsCardProps {
  /** Amount of automatic savings made today */
  amount?: string;
  /** Percentage of daily budget saved automatically */
  percentage?: string;
...
/**
 * Component for displaying automatic savings information
 */
export const AutomaticSavingsCard: React.FC<AutomaticSavingsCardProps> = ({
  amount = '0',
  percentage = '0',
  monthSum = '0',
  isLoading = false,
  ...
  return (
    <VStack className={`bg-background-50 p-4 rounded-lg border border-border-200 shadow-sm ${className}`}>
      ...
      <Text className="text-sm font-medium text-text-600 mb-2">
        Automatyczne oszczędności dzisiaj
      </Text>
